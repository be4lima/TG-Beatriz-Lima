...
Existem pré-requisitos para a regressão linear: 
...
3. Normalidade: para um valor fixo de X, Y é uma variável aleatória com distribuição norma. Os resíduos também devem ser normalmente distribuídos;

3. Independência dos resíduo: como Yi e Yj são valores estatisticamente independentes (falta de correlação), os resíduos também deverão ser independentes. Uma forma de verificar isso é realizar o teste de Durbin-Watson.
...
#Retirando outliers dos resíduos
...
Para a análise residual, é necessário investigar se a relação entre X e Y não é linear, se os erros possuem variância constante (homocedasticidade), se os erros são correlacionados, se os erros não são normalmente distribuídos, se o modelo não possui ajuste bom a uma ou mais observações e uma ou mais covariáveis não foram incluídas no modelo.
...
De acordo com o teste de Shapiro, é possível confirmar que p-value tem valor menor que 0,05 e, portanto, possui distribuição diferente da normal. <- rever isso

...
A **Análise Exploratória de Dados** (EDA) é uma forma de utilizar ferramentas gráficas e estatísticas apropriadas da linguagem R na exploração de dados. A partir da visualização, transformação e modelagem de dados, essa análise explora os dados de forma sistemática [@wickhman22].

Para aplicar a EDA, de acordo com @wickhman22, basta inicialmente gerar questões sobre os dados, utilizar as funcionalidades do R para encontrar respostas e utilizar o que foi aprendido para gerar novas questões. Portanto, essa análise é um ciclo iterativo.
...
lativas das categorias da variável[@morettin2017; @bruce2019].

As representações gráficas para viariáveis quantitativas apresentam maior variedade,
porque, além dos gráficos usados para variáveis qualitativas, ainda é possível realizar gráficos de linhas, de dispersão, histogramas, de ramo-e-folhas e box-plots [@morettin2017; @favero2017].

A representação gráfica de pontos formados pela intersecção no eixo x e y ligados por
uma linha reta é o gráfico de linhas. Conforme apresentado por @favero2017, esse gráfico
...
representam os primeiros dígitos da observação. À direita ficam as folhas, as quais representam os últimos dígitos da observação. Ou seja, segundo @favero2017, "os ramos são representados em uma única coluna e seus diferentes valores ao longo de várias colunas". Ambos devem estar ordenados de forma crescente. Quando um ramo apresenta muitas folhas, é possível afirmar que existe maior incidência dele. Se o número de folhas for grande, pode-se criar duas ou mais linhas de um mesmo ramo [@favero2017; @morettin2017].

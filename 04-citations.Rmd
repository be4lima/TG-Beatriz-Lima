...
Dada uma série temporal {$y_t^T_{t=1}$}, o operador de diferenças transforma a série original em uma nova série formada por diferenças sucessivas:
...
Para realizar o teste da raiz unitária, os mais utilizados são o de teste de Ljung-Box e o teste Aumentado de Dickey-Fuller (ADF).
...
## Médias Móveis
O modelo de médias móveis MA(q) modela a partir das defasagens do termo de erro $\epsilon_t$, sendo assim uma combinação linear de ruídos brancos ($\epsilon+t$ tem média zero, variância constante e não-autocorrelacionado).
...
A determinação das ordens p e q do modelo pode ser realizada a partir da análise gráfica das função de autorrelação FAC (Função de Autocorrelação) e FACP (Função de Autocorrelação Parcial).
...
https://rpubs.com/frank-pinho/522364#:~:text=A%20classe%20de%20modelos%20autorregressivos,constante%20e%20n%C3%A3o%2Dautocorrelacionado).

https://rpubs.com/hudsonchavs/modeloma#:~:text=A%20classe%20de%20modelos%20de,modelar%20o%20retorno%20em%20t.
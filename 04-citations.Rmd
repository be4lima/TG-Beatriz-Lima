...
De acordo com @filho2009, existem métricas diferentes de acordo com cada autor:
...
A partir da [regressão](https://towardsdatascience.com/laymans-introduction-to-linear-regression-8b334a3dab09), é possível obter a relação matemática que descreva a relação entre duas ou mais variáveis. A análise de regressão é uma coleção de ferramentas estatísticas que permite a modelagem e inferência de uma variável dependente (y) com uma ou mais variáveis independentes (x). No caso da **regressão linear simples**, somente existe uma variável independente; para a regressão linear múltipla, mais de uma. Dessa forma, o formato básico do modelo de regressão linear é:
...
De acordo com @montgomery2021, o R² pode trazer interpretações errôneas, já que sempre é possível fazer com que R² seja unitário realizando a adição de mais termos ao modelo. Ou seja, R² aumenta se for adicionado uma nova variável ao modelo, mas isso não indica que o modelo esteja mais adequado.
...
```{r 4-1,message=FALSE,warning=FALSE, eval=FALSE}
...
```{r 4-5,message=FALSE,warning=FALSE}
...
De acordo com @montgomery2021, "*A análise dos resíduos é frequentemente útil na verificação da suposição de que os erros sejam distribuídos de forma aproximadamente normal, com variância constante, assim como na determinação da utilidade dos termos adicionais no modelo*". Dessa forma, abaixo será realizada a análise residual.

```{r 4-6,message=FALSE,warning=FALSE}
...
A [Distância de Cook](https://towardsdatascience.com/identifying-outliers-in-linear-regression-cooks-distance-9e212e9136a#:~:text=One%20method%20that%20is%20often,the%20ith%20observation%20is%20removed) informa o quanto um caso é capaz de influenciar o modelo de regressão. Portanto, o gráfico estima a influência de cada observação no modelo e, novamente, as observações 48, 50 e 51 são extremas. 

```` {r 4-7,message=FALSE,warning=FALSE}
...
Pelo [Teste de Shapiro](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/shapiro.test), é possível verificar que o p-value é menor que 0,05, portanto, o valor não é adequado. O gráfico Normal Q-Q serve como uma contra-prova, também mostrando que os dados não são normalmente distribuídos e, dessa forma, não são adequados.
...
```{r 4-8,message=FALSE,warning=FALSE}
...
```{r 4-9,message=FALSE,warning=FALSE}

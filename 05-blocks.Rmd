...
# Última observação significativa: 2
```
A Função de Correlação (FAC), que determina a ordem das médias móveis (q), indica que a última observação significativa é a 2. E a FACP, que determina a ordem p da componente autoregressiva (AR), indica que a última observação significativa é a 2 também.

A partir disso, o modelo que será utilizado é o ARIMA (2,2,1).
```{r 5-31}
...
```
Conforme apresentado, a função `auto.arima` indica o modelo que escolhemos e, dessa forma, pode-se afirmar que o modelo escolhido é o que minimiza os critérios AIC e BIC.

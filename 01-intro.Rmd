...
Após realizar a sumarização numérica de todos os dados, agora será focado nos dados de interesse: precipitação e ano. Primeiramente, será encontrada a média de precipitação por ano analisado.

```{r}

#Média das precipitações por ano

PrepAno <- aggregate(met1[,3], list((met1$ANO)), mean, na.rm=TRUE)


```

Com o objetivo de analisar a graficamente os valores de precipitação por ano, será utilizado um gráfico de barras (`barplot()`).

```{r}
#ggplot(PrepAno)+geom_bar(aes(x=PrepAno$x, #y=PrepAno$Group.1))+geom_errorbar(aes(x=PrepAno$x, #ymin=PrepAno$Group.1-sd, ymax=PrepAno$Group.1+sd, width=0.4, #colour="orange"))

barplot(PrepAno$x,
        main="Precipitacao média (mm) por ano",
        xlab="Anos",
        ylab= "Precipitacao (mm)",
        col= "blue",
        names.arg = PrepAno$Group.1)

```

É possível analisar que houve um aumento do volume de chuvas no ano de 2015, se comparado com o ano de 2014. O valor médio de precipitação de 2016 é ligeiramente maior que a média de 2014. Porém, não é possível afirmar que este acontecimento é decorrente somente como consequência do El Niño, porque muitos outros fatores não analisados aqui podem ter influenciado no aumento do volume de chuvas. 
